...
class evaluaciones extends conexion {
    ...
    ***************************************************/
    public function listaEvaluaciones($tipo){

        $condicion="where estatus=1 ";
        if($tipo != 'todas'){
            $selecttipo =" and codEvaluaiocnesHO = '$tipo'";
            $bandera=1;
         }else{
            $selecttipo =" ";
         }

        $query = "select * from dg_evaluaciones_ho_informes $condicion $selecttipo order by 2";
//        echo $query; die;
            ...
            if($arrayToken){
               //valida los campos obligatorios
                if  (
                    ...
                    //(!isset($datos['header']['codigo']))||  //***NO SE ESTA GUARDANDO EN LA db */
                    (!isset($datos['header']['sector']))||
                    (!isset($datos['header']['fecha']))||
                    (!isset($datos['header']['complejo']))||
                    (!isset($datos['header']['gerencia']))||
                  //  (!isset($datos['header']['area']))||  //FALTA LLENARLO EN LA VISTA
                        ...
                        if($this->tipoIncidenciaId==1){
                            ...
                            foreach ($datos['hallazgosNew'] as $hallazgosNew  ){
                                ...
                                foreach ($hallazgosNew['hallazgorecomendaciones'] as $key){   
                                    ...
                                    $responsableHallazgo=$key["incidencia_responsable"];
                        ...
                        elseif($this->tipoIncidenciaId==3){
                            ...
                            foreach ($datos['hallazgosNew'] as $hallazgosNew  ){
                             //print_r($hallazgosNew); die;  
                                ...
                                $this->tipo = $hallazgosNew['tipo'];
                                $this->heredadas = '0';
                                $this->danoAmbiental = $hallazgosNew['danoAmbiental'];
                                $this->causaEvento = $hallazgosNew['causaEvento'];
