<?php
 ...
 require_once 'clases/ambienteInforme.class.php';

 $_respuestas = new respuestas();
 $_informeAmbiente = new informeAmbiente();

 if ($_SERVER['REQUEST_METHOD'] == 'GET') {// Get READ
     if (isset($_GET['idInforme'])) {
         $idInforme = $_GET['idInforme'];

         $listaInforme = $_informeAmbiente->detalleInforme($idInforme);
         // prepara salida del ws
         header('Content-Type: application/json;charset=utf-8');
         echo json_encode($listaInforme);
     ...
     $datosArray = $_informeAmbiente->post($postBody);
